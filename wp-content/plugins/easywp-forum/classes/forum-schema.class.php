...
class ForumSchema
{
    ...
    private function create_module_table(){
        $create_module_query = "CREATE TABLE ".FORUM_MODULE_TABLE." (
                ...
                ";
        $this->execute_query($create_module_query,FORUM_MODULE_TABLE);
    }

    public function create_forum_category_table(){
        $create_forum_category_query = "CREATE TABLE ".FORUM_CATEGORY_TABLE." (
                    ...
                    FOREIGN KEY (parent_category_id) REFERENCES ".FORUM_CATEGORY_TABLE."(category_id) ON DELETE CASCADE ON UPDATE CASCADE,
                    FOREIGN KEY (user_id) REFERENCES wp_users(ID)
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
                ";
    ...
    private function create_forum_table(){
        $create_forum_query = "CREATE TABLE ".FORUM_TABLE." (
                ...
                ";
        $this->execute_query($create_forum_query,FORUM_TABLE);
    }

    public function create_document_category_table(){
        $create_document_category_query = "CREATE TABLE ".FORUM_DOCUMENT_CATEGORY_TABLE." (
                    category_id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
                    module_id BIGINT(20) UNSIGNED NOT NULL,
                    parent_category_id INT(11) UNSIGNED DEFAULT 0,
                    user_id BIGINT(20) UNSIGNED NOT NULL,
                    ...
                    PRIMARY KEY (category_id),
                    FOREIGN KEY (module_id) REFERENCES ".FORUM_MODULE_TABLE."(module_id) ON DELETE CASCADE ON UPDATE CASCADE,
                    FOREIGN KEY (parent_category_id) REFERENCES ".FORUM_DOCUMENT_CATEGORY_TABLE."(category_id) ON DELETE CASCADE ON UPDATE CASCADE,
                    FOREIGN KEY (user_id) REFERENCES wp_users(ID)
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
                ";
        $this->execute_query($create_document_category_query,FORUM_DOCUMENT_CATEGORY_TABLE);
    ...
    private function create_document_table(){
        $create_board_document_query = "CREATE TABLE ".FORUM_DOCUMENT_TABLE." (
                ...
                ";
        $this->execute_query($create_board_document_query,FORUM_DOCUMENT_TABLE);
    }

    private function create_comment_table(){
        $create_document_category_query = "CREATE TABLE ".FORUM_COMMENT_TABLE." (
                    ...
                    FOREIGN KEY (module_id) REFERENCES ".FORUM_MODULE_TABLE."(module_id) ON DELETE CASCADE ON UPDATE CASCADE,
                    FOREIGN KEY (forum_id) REFERENCES ".FORUM_TABLE."(forum_id) ON DELETE CASCADE ON UPDATE CASCADE,
                    FOREIGN KEY (document_id) REFERENCES ".FORUM_DOCUMENT_TABLE."(document_id) ON DELETE CASCADE ON UPDATE CASCADE,
                    FOREIGN KEY (parent_comment_id) REFERENCES ".FORUM_COMMENT_TABLE."(comment_id) ON DELETE CASCADE ON UPDATE CASCADE,
                    FOREIGN KEY (user_id) REFERENCES wp_users(ID)
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
                ";
        $this->execute_query($create_document_category_query,FORUM_COMMENT_TABLE);
    }

    private function execute_query($query, $table){
        $create_table = $this->forum_wpdb->query($query);

        if ($create_table === false)
        {
            $create_table_err = _e('There was an error creating the table '.$table, __MODULE_NAME__);
            ForumContext::log($create_table_err);
        }
    }

    private function drop_table_by_name($table){
        $query = "DROP TABLE IF EXISTS ".$table;
        $drop_table = $this->forum_wpdb->query($query);

        if ($drop_table === false) {
            $drop_table_err = _e('There was an error dropping the table '.$table, __MODULE_NAME__);
            ForumContext::log($drop_table_err);
        }
    }
